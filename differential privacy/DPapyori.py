...
# Meta informations for original author of fork.
# __version__ = '1.1.2'
# __author__ = 'Yu Mochizuki'
# __author_email__ = 'ymoch.dev@gmail.com'

__version__ = '1.0.0'
__author__ = 'Przemek Warias'
__author_email__ = 'pwarias@hawk.iit.edu'
...
class TransactionManager(object):
    ...
    def calc_support(self, items, noise):
        """
        ...
        return (float(len(sum_indexes)) + float(noise)) / self.__num_transaction
...
def gen_support_records(transaction_manager, min_support, **kwargs):
    ...
    while candidates:
        relations = set()
        kappa = len(candidates)
        for relation_candidate in candidates:
            support = transaction_manager.calc_support(relation_candidate, G(epsilon/kappa))
...
def gen_ordered_statistics(transaction_manager, record):
    ...
    for base_length in range(len(items)):
        for combination_set in combinations(sorted_items, base_length):
            ...
            # The confidence and lift here are discarded so the value of 2 is irrelevant
            # Need to fix
            confidence = (
                record.support / transaction_manager.calc_support(items_base, 2)) 
            lift = confidence / transaction_manager.calc_support(items_add, 2)
...
def G(epsilon_div_delta, size=None):
    p = 1 - np.exp(-epsilon_div_delta)
    Z = np.random.geometric(p, size) - np.random.geometric(p, size)
    return Z
...
def apriori(transactions, **kwargs):
    ...
    # Need length of itemsets to not be None to ensure epsilon/(itemset length) makes sense
    ...
    support_records = _gen_support_records(
        transaction_manager, min_support, max_length=max_length, epsilon=epsilon)
    ...
    for support_record in support_records:
        ...
        # DP modification to inject noise into support record
        if not ordered_statistics:
            continue
        temp = [support_record.items, support_record.support]
        yield tuple(
            temp)
